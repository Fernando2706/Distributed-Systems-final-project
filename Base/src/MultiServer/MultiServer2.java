...
public class MultiServer2 {
		...
		public static void main(String[] args) {
			try {
				ServerSocket listen = new ServerSocket(GlobalFunctions.getPort("SERVER2"));
	            Semaphore sem = new Semaphore(1);
				while(true) {
					System.out.println("Waiting server 2...");
					Socket socket = listen.accept();
					new ConnectionServer2(socket,sem);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
...
class ConnectionServer2 extends Thread{
 ...
 public ConnectionServer2(Socket socketCentral,	 Semaphore sem ) {
		 try {
	 ...
	 public void run() {
		 ...
		 public void update(String text) {
	         ...
	         GlobalFunctions.updateLog("MultiServer2.txt",(formatter.format(date)+" "+text));
	         this.sem.release();
		 }
